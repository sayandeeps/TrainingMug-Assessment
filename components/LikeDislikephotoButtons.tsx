...
import { incrementLikes, incrementDislikes } from '../store/features/likeDislikephotoSlice';

interface LikeDislikephotoButtonsProps {
  postId: number;
}

const LikeDislikephotoButtons: React.FC<LikeDislikephotoButtonsProps> = ({ postId }) => {
  const dispatch = useDispatch();
  const { likes, dislikes } = useSelector((state: RootState) => state.likeDislikephoto[postId] || { likes: 0, dislikes: 0 });
...
export default LikeDislikephotoButtons;
